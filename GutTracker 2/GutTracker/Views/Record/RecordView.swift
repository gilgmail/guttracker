...
struct RecordView: View {
    ...
    // Today's data
    @Query(filter: #Predicate<BowelMovement> {
        $0.timestamp >= Calendar.current.startOfDay(for: .now)
    }, sort: \BowelMovement.timestamp, order: .reverse)
    private var todayBowelMovements: [BowelMovement]
    
    @Query(filter: #Predicate<MedicationLog> {
        $0.timestamp >= Calendar.current.startOfDay(for: .now)
    })
