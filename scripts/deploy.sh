#!/bin/bash
# GutTracker — 編譯並安裝到 Gil-Golden (iPhone 16 Pro)
...
BUILD_LOG="$LOG_DIR/deploy_${TIMESTAMP}.log"
...
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BOLD}  GutTracker → $DEVICE_NAME${NC}"
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
# ── 編譯 ──
echo -e "${CYAN}[編譯]${NC} 開始編譯 (device build)..."
BUILD_START=$(date +%s)
...
echo -e "${GREEN}${BOLD}━━━ BUILD SUCCEEDED ━━━${NC}  (${DURATION}s)"
echo ""

# ── 找到 .app ──
APP_PATH=$(find ~/Library/Developer/Xcode/DerivedData/guttracker-*/Build/Products/Debug-iphoneos -name "guttracker.app" -maxdepth 1 2>/dev/null | head -1)
if [ -z "$APP_PATH" ]; then
    echo -e "${RED}[錯誤]${NC} 找不到編譯產出的 .app"
    exit 1
fi
...
if [ "$LAUNCH" = true ]; then
    ...
    if xcrun devicectl device process launch --device "$DEVICE_ID" com.gilko.guttracker 2>/dev/null; then
